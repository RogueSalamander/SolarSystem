...
public class PlanetRing : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < segments + 1; i++)
        {
            ...
            vertices[i * 2] = vertices[i * 2 + halfway] = new Vector3(x, 0, z) * (innerRadius + thickness);
            vertices[i * 2 + 1] = vertices[i * 2 + 1 + halfway] = new Vector3(x, 0, z) * innerRadius;
            uv[i * 2] = uv[i * 2 + halfway] = new Vector2(progress, 0f);
            uv[i * 2] = uv[i * 2 + 1 + halfway] = new Vector2(progress, 1f);
